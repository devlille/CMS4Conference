...
export async function saveCompany(firestore: FirebaseFirestore.Firestore, storage: Storage, companyId: string, event: Company, context: any) {
  ...
  if (event.logoUrl == '' || event.logoUrl == undefined || event.logoUrl == null) {
    console.log("Partner doesn't have a logo.");
    return;
  }
  if (event.siteUrl == '' || event.siteUrl == undefined || event.siteUrl == null) {
...
async function _genPng(storage: Storage, c4hId: string, companyId: string, logoUrl: string, size: number) {
