...
export async function onDocumentChange(firestore: FirebaseFirestore.Firestore, before: Company, after: Company, id: string, configuration: Configuration) {
  ...
  if (status.generated === StatusEnum.PENDING) {
    ...
    ((status.generated === StatusEnum.DONE && beforeStatus.generated !== StatusEnum.RETRY) || status.generated === StatusEnum.RETRY)
  ) {
    await generateAndStoreProformaInvoiceAndConvention(after, id, configuration);
    await generateAndStoreInvoice(firestore, after, id, configuration);

    return document.update({
      ...partnershipGenerated(after, id, configuration, status.generated === StatusEnum.DONE)
    });
    ...
    return document.update({
      ...partnershipValidated(after, id, configuration, status.validated === StatusEnum.DONE)
    });
