...
describe("Testing users (users/{userId}) security rule", () => {
  ...
  beforeAll(async () => {
    ...
    await testEnv.withSecurityRulesDisabled(async (context) => {
      ...
      await firestoreWithoutRule
        .collection("companies-2025")
        .doc("1")
  ...
  it("should not be able to access to the collection", async () => {
    ...
    await testing.assertFails(
      getDocs(collection(user.firestore(), "companies-2025"))
    );
  });

  it("should be able to access to the collection", async () => {
    const user = testEnv.authenticatedContext("manu", {
      email: "manu@devlille.fr",
    });
