...
export class FormComponent {
  ...
  private initFormGroup(company: Company) {
    return new FormGroup({
      ...
      officialName: new FormControl(
        { value: company.officialName, disabled: false },
        []
      ),
      ...
      siret: new FormControl({ value: company.siret, disabled: false }, [
        Validators.required,
        Siret(),
      ]),
      representant: new FormControl(
        { value: company.representant, disabled: false },
        [Validators.required]
      ),
      ...
      role: new FormControl({ value: company.role, disabled: false }, [
        Validators.required,
      ]),
  ...
  onSubmitForm() {
    ...
    if (!!this.place) {
      ...
      if (
        !!this.place?.address_components?.find(
          (a) => a.types.indexOf('street_number') >= 0
        )
      ) {
        streetNumber =
          this.place?.address_components?.find(
