...
type FilterByPackValueType = 'Platinium' | 'Gold' | 'Silver' | 'Bronze' | 'all';
type filterByTypeValueType = 'esn' | 'other' | 'all';

@Component({
    selector: 'cms-dashboard',
    standalone: true,
    imports: [CommonModule, MatRadioModule, MatTableModule, FormsModule, MatCardModule, MatButtonModule, MatSortModule],
...
export class DashboardComponent implements AfterViewInit {
    ...
    displayedColumns: string[] = ['creationDate', 'name', 'sponsoring', 'secondSponsoring', 'pending', 'action'];
    ...
    filterByValue(filterValue: FilterValueType) {
        ...
        this.sortAndFilter(this.originalPartners, this.filterValue, this.filterByPackValue, this.filterByTypeValue);
    }
    ...
    private sortAndFilter(
        ...
        this.dataSource = new MatTableDataSource(
            originalPartners
                ...
                .filter(partner => filterValue === 'all' || partner.pending!.indexOf(filterValue) >= 0)
                .filter(partner => {
                    return type === 'all' || partner.type === type || (!partner.type && type === 'other');
                })
        );
        this.dataSource.sort = this.sort;
    }
